...
export function getFilteredAppearanceList(characters: CharacterAsset[], clipCharacters: ClipCharacterRef[]): string {
    ...
    return characters
        ...
        .map((c) => {
            ...
            if (appearances.length === 0) return `${c.name}: ["Initial Look"]`
            const appearanceNames = appearances.map((app) => app.changeReason || 'Initial Look')
    ...
    return characters
        ...
        .map((c) => {
            ...
            return appearances.map((app) => {
                const appearanceName = app.changeReason || 'Initial Look'
