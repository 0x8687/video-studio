...
import { useUploadLocationImage } from '@/lib/query/mutations/asset-hub-location-mutations'
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (mode === 'asset-hub') {
                ...
                if (uploadFile && createdLocationId) {
                    try {
                        await uploadLocationImage.mutateAsync({
                            file: uploadFile,
                ...
                <div className="p-6 overflow-y-auto flex-1">
                    ...
                    <div className="space-y-5">
                        ...
                        {mode === 'asset-hub' && (
                            <div className="space-y-2">
                                <label className="glass-field-label block">
                                    {t('location.optionalImageUpload')}
                                </label>
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className="glass-btn-base glass-btn-soft px-3 py-2 rounded-lg text-sm disabled:opacity-40 disabled:cursor-not-allowed"
                                    >
                                        {t('location.optionalImageUploadButton')}
                                    </button>
                                    {uploadFile && (
                                        <div className="flex items-center gap-2 text-xs text-[var(--glass-text-secondary)]">
                                            <span className="truncate max-w-[160px]">{uploadFile.name}</span>
                                            <button
                                                type="button"
                                                onClick={() => setUploadFile(null)}
                                                className="text-[var(--glass-tone-danger-fg)] hover:underline"
                                            >
                                                {t('common.clear')}
                                            </button>
                                        </div>
                                ...
                                <p className="text-xs text-[var(--glass-text-tertiary)]">
                                    {t('location.optionalImageUploadHint')}
                                </p>
                            </div>
                        )}
                    </div>
