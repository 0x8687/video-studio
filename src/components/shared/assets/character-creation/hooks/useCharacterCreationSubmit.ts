...
import { useUploadCharacterImage } from '@/lib/query/mutations/asset-hub-character-mutations'
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (mode === 'asset-hub') {
        ...
        if (uploadFile && createdCharacterId) {
          const appearanceIndex = createdAppearanceIndex ?? 0
          try {
            await uploadCharacterImage.mutateAsync({
              file: uploadFile,
